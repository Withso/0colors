// ─── Shared Color Space Conversion Utilities ────────────────────
// Centralized to avoid bundler name-collision issues with duplicate
// function declarations across modules.
...
// ─── sRGB linearization / gamma helpers ────────────────────────
...
function linearToSrgb(val: number): number {
  const c = Math.max(0, Math.min(1, val)); // clamp to sRGB gamut
  return c <= 0.0031308 ? c * 12.92 : 1.055 * Math.pow(c, 1 / 2.4) - 0.055;
}

// ─── OKLab / OKLCH matrices (Björn Ottosson spec) ─────────────
// M1: linear sRGB → LMS (direct, NOT through XYZ)
...
/** HSL -> OKLCH (L 0-100, C 0-40 raw×100, H 0-360)
 *  Uses the OKLab spec's direct linear-sRGB → LMS matrix (M1). */
 ...
 *  Input: L 0-100, C 0-100 display-scale (raw = C/100*0.4), H 0-360.
 *  Returns unclamped linear sRGB values for gamut-aware processing. */
function oklchToLinearSrgb(lightness: number, chroma: number, hue: number): { r: number; g: number; b: number } {
  const okL = lightness / 100;
  const rawC = chroma / 100 * 0.4; // display 0-100 → raw 0-0.4
  const hRad = hue * Math.PI / 180;
  const labA = rawC * Math.cos(hRad);
  const labB = rawC * Math.sin(hRad);

  // OKLab → LMS^(1/3) (M2_inv)
...
/** OKLCH (L 0-100, C 0-100, H 0-360) -> HSL
 *  Uses the OKLab spec's LMS → linear-sRGB matrix (M1_inv, single step). */
...
/** OKLCH → sRGB { r, g, b } (0-255, clamped to gamut).
 *  Input: L 0-100, C 0-100 display-scale, H 0-360.
 *  Direct conversion without HSL intermediate — maximally accurate for hex. */
