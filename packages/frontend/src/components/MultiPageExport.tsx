                ...
                <DropdownMenuContent align="start" className="w-[180px] bg-[#111] border-[#1a1a1a] p-1 shadow-lg">
                  {projectThemes.filter(t => selectedThemeIds.has(t.id)).map(theme => (
                    <DropdownMenuItem
                      ...
                      className="text-xs cursor-pointer focus:bg-[#ffffff]/[0.06] focus:text-[#ededed]"
                    >
                      <span className={`truncate ${theme.id === previewThemeId ? 'text-[#ededed]' : 'text-[#888]'}`} title={theme.name}>
                        {theme.name}
                      </span>
                      {theme.isPrimary && <span className="ml-2 text-[10px] text-[#444]">(Primary)</span>}
                      {theme.id === previewThemeId && <Check className="h-3 w-3 text-[#555] ml-auto" />}
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
              </DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-[220px] bg-[#111] border-[#1a1a1a] p-1 shadow-lg">
              {(Object.keys(formatLabels) as CodeFormat[]).map(fmt => (
                <DropdownMenuItem
                  ...
                  className="flex items-center justify-between text-xs cursor-pointer focus:bg-[#ffffff]/[0.06] focus:text-[#ededed]"
                >
                  <div className="flex flex-col">
                    <span className={fmt === selectedFormat ? 'text-[#ededed]' : 'text-[#888]'}>
                      {formatLabels[fmt]}
                    </span>
                    <span className="text-[10px] text-[#444]">{formatDescriptions[fmt]}</span>
                  </div>
                  {fmt === selectedFormat && <Check className="h-3 w-3 text-[#555]" />}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          <ScrollArea className="flex-1">
            ...
            <div className="px-4 pt-4 pb-2">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[11px] text-[#666] uppercase tracking-widest">Pages</span>
                <button
              ...
              {projectPages.map(page => (
                ...
                >
                  ...
                  <span className={`text-[12px] flex-1 transition-colors truncate ${
                    selectedPageIds.has(page.id) ? 'text-[#999]' : 'text-[#444]'
            ...
            <div className="px-4 pt-3 pb-2">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[11px] text-[#666] uppercase tracking-widest">Themes</span>
                <button
            ...
            <div className="px-2 pb-4">
              {projectThemes.map(theme => (
                ...
                >
                  ...
                  />
                  <span className={`text-[12px] flex-1 transition-colors truncate ${
                    selectedThemeIds.has(theme.id) ? 'text-[#999]' : 'text-[#444]'
                  }`} title={theme.name}>{theme.name}</span>
                  {theme.isPrimary && (
                    <span className="text-[10px] text-[#444]">(Primary)</span>
                  )}
                </div>
              ))}
            </div>
              ...
              <>
                ...
                <div className="px-4 pt-3 pb-2">
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <Hash className="h-3 w-3 text-[#555]" />
                    <span className="text-[11px] text-[#666] uppercase tracking-widest">Show as Hex</span>
                  </div>
                  ...
                  {([
                    ...
                    >
                      ...
                      <span className={`text-[12px] flex-1 transition-colors ${
                        hexOverrideSpaces.has(key) ? 'text-[#999]' : 'text-[#444]'
                      }`}>{label}</span>
                    </div>
                  ))}
                </div>
