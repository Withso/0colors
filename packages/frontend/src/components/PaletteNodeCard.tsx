...
function CurveVisualizer({
  ...
  return (
    ...
    >
      {/* Grid lines */}
      <line x1={PAD} y1={PAD} x2={PAD} y2={H - PAD} stroke="#1a1a1a" strokeWidth="0.5" />
      <line x1={PAD} y1={H - PAD} x2={W - PAD} y2={H - PAD} stroke="#1a1a1a" strokeWidth="0.5" />
      {/* 50% lightness guide */}
      <line
        x1={PAD}
      ...
      {/* Axis labels */}
      <text x={PAD} y={H - 2} fill="#444" fontSize="7" fontFamily="var(--font-mono)">L</text>
      <text x={W - PAD - 6} y={H - 2} fill="#444" fontSize="7" fontFamily="var(--font-mono)">R</text>
    </svg>
  );
}
  ...
  return (
    ...
    >
      ...
      <Island isSelected={isSelected} isMultiSelected={isMultiSelected} noPadding>
        ...
        <div className="border-t border-[#141414] transition-opacity">
          <div
            ...
            onMouseDown={(e) => e.stopPropagation()}
          >
            <span className="text-[10px] text-[#555] uppercase tracking-widest" style={{ fontFamily: 'var(--font-sans)' }}>Name</span>
            <div className="flex items-center gap-1.5">
              <button
                onClick={(e) => { e.stopPropagation(); onUpdateNode(node.id, { paletteNameLocked: !node.paletteNameLocked }); }}
                className="w-5 h-5 rounded flex items-center justify-center transition-colors hover:bg-[#1a1a1a]"
            ...
            <div className="px-4 pb-3">
              <Select value={curveType} onValueChange={handleCurveChange}>
                ...
                <SelectContent className="bg-[#111] border-[#222]">
                  ...
                  <SelectItem value="ease-in-out" className="text-[#ededed] text-xs">Ease In-Out</SelectItem>
                  <SelectItem value="sine" className="text-[#ededed] text-xs">Sine</SelectItem>
                  <SelectItem value="exponential" className="text-[#ededed] text-xs">Exponential</SelectItem>
                  <SelectItem value="material" className="text-[#ededed] text-xs">Material</SelectItem>
                  <SelectItem value="custom" className="text-[#ededed] text-xs">Custom</SelectItem>
                </SelectContent>
              </Select>
            ...
            <div className="px-4 pb-3">
              <div className="relative pt-1 pb-1">
                ...
                <div className="flex justify-between mt-1">
                  <span className="text-[10px] text-[#555]" style={{ fontFamily: 'var(--font-mono)' }}>L:{lightnessStart}%</span>
                  <span className="text-[10px] text-[#555]" style={{ fontFamily: 'var(--font-mono)' }}>L:{lightnessEnd}%</span>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="px-4 pb-3">
              ...
              {satMode === 'manual' && (
                <div className="relative pt-1 pb-1">
                  ...
                  />
                  <div className="flex justify-between mt-1">
                    <span className="text-[10px] text-[#555]" style={{ fontFamily: 'var(--font-mono)' }}>S:{satStart}%</span>
                    <span className="text-[10px] text-[#555]" style={{ fontFamily: 'var(--font-mono)' }}>S:{satEnd}%</span>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="border-t border-[#141414]">
          <button className="w-full flex items-center justify-between px-4 py-2.5 cursor-pointer hover:bg-[#111] transition-colors" onClick={(e) => { e.stopPropagation(); toggleSection('hueShift'); }} onMouseDown={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-[#ededed]" style={{ fontFamily: 'var(--font-mono)' }}>{hueShift > 0 ? '+' : ''}{hueShift}°</span>
              <ChevronDown className={`w-3.5 h-3.5 text-[#555] transition-transform ${expandedSections.hueShift ? 'rotate-180' : ''}`} />
            </div>
          </button>
          {expandedSections.hueShift && (
            <div className="px-4 pb-3">
              <input type="range" min="-30" max="30" value={hueShift} onChange={(e) => onUpdateNode(node.id, { paletteHueShift: parseInt(e.target.value) })} className="w-full h-2 bg-[#111] rounded-full appearance-none cursor-pointer palette-range-slider" onMouseDown={(e) => e.stopPropagation()} />
            </div>
          )}
        </div>
        ...
        <div className="border-t border-[#141414]">
          <button className="w-full flex items-center justify-between px-4 py-2.5 cursor-pointer hover:bg-[#111] transition-colors" onClick={(e) => { e.stopPropagation(); toggleSection('pattern'); }} onMouseDown={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-4">
              ...
              <span className="text-[10px] text-[#555] uppercase tracking-widest" style={{ fontFamily: 'var(--font-sans)' }}>Shades</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-[#ededed]" style={{ fontFamily: 'var(--font-mono)' }}>{shadeCount}</span>
              <ChevronDown className={`w-3.5 h-3.5 text-[#555] transition-transform ${expandedSections.pattern ? 'rotate-180' : ''}`} />
            </div>
          </button>
                ...
                <div>
                  <Select value={namingPattern} onValueChange={(value) => onUpdateNode(node.id, { paletteNamingPattern: value as any })}>
                    ...
                    <SelectContent className="bg-[#111] border-[#222]">
                      ...
                      <SelectItem value="a-z" className="text-[#ededed] text-xs">a-z</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  <input type="range" min="2" max="20" value={shadeCount} onChange={(e) => onUpdateNode(node.id, { paletteShadeCount: parseInt(e.target.value) })} className="w-full h-2 bg-[#111] rounded-full appearance-none cursor-pointer palette-range-slider" onMouseDown={(e) => e.stopPropagation()} />
                </div>
              </div>
            </div>
          )}
      ...
      >
        ...
        >
          <Plus className={`w-3 h-3 ${isStructurallyLocked ? 'text-[#555]' : isWireHovered && wireStartButtonType === 'right' ? 'text-white' : 'text-[#ededed]'}`} strokeWidth={3} />
        </button>
      </div>
