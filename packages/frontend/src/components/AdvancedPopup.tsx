...
function TokenPill({
  ...
  const color = TOKEN_COLORS[token.type] || '#666';
  ...
  return (
    ...
    >
      {isLocked && (
        <svg width="8" height="8" viewBox="0 0 16 16" fill="none" className="shrink-0 opacity-70">
    ...
    >
      ...
      >
        {Object.entries(groupedItems).map(([category, items]) => (
          <div key={category}>
            <div
              className="px-3 py-1.5 text-[9px] tracking-widest uppercase text-[#444] select-none sticky top-0"
              ...
              return (
                ...
                >
                  ...
                  {item.tokenType === 'reference' && !item.refProperty && !item.isBareRef && (
                    <ChevronRight size={10} className="ml-auto text-[#333]" />
                  )}
...
function ConditionRowEditor({
  ...
  const renderGap = (targetCursorPos: number, key: string) => (
    <span
      key={key}
      className="inline-block w-[4px] h-[18px] shrink-0 cursor-text"
      onClick={(e) => {
        e.stopPropagation();
  ...
  const renderInput = (key: string) => (
    <input
      key={key}
      ref={inputRef}
      className="bg-transparent border-none outline-none text-[11px] font-mono text-[#888] max-w-full"
  ...
  return (
    <div ref={rowRef} className="group relative" data-palette-open={showPalette || undefined}>
      <div
      ...
      >
        ...
        <div
          ...
          className="flex-1 flex flex-wrap items-center gap-[3px] min-h-[22px] cursor-text relative"
          onClick={(e) => {
      ...
      {ctxMenu && createPortal(
        <div
          className="fixed z-[9999] min-w-[120px] py-1 rounded-md shadow-xl"
          style={{
            left: ctxMenu.x,
            top: ctxMenu.y,
            background: '#1e1e1e',
        ...
        >
          <button
            className="w-full px-3 py-1 text-left text-[11px] text-[#ccc] hover:bg-white/[0.08] flex items-center gap-2 cursor-pointer"
            onClick={() => {
              _expressionClipboard = getSelectedTokens();
              setCtxMenu(null);
            }}
...
function OutputNameInput({ value, defaultName, onChange }: {
  ...
  return (
    <input
      className="bg-transparent border-none outline-none text-[9px] font-mono text-[#555] w-full min-w-[30px] max-w-[60px]"
      value={displayValue}
      onChange={(e) => {
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5">
          <span className="text-[11px] text-[#555] tracking-wide uppercase select-none">
            {channelDef.label}
          </span>
          {hasRows && (
            <button
              className="flex items-center justify-center w-4 h-4 rounded transition-colors cursor-pointer hover:bg-white/[0.08]"
      ...
      <div className="flex-1 overflow-y-auto px-1 py-2 space-y-0 min-h-0">
        {logic.rows.map((row, index) => {
          ...
          return (
            <div key={row.id}>
              ...
              {/* Per-row output box */}
              <div
                className="mx-2 mb-2 flex items-center justify-between gap-1 px-2 py-1 rounded"
                style={{
              ...
              >
                <div className="flex items-center gap-1 min-w-0">
                  <span className="text-[8px] text-[#333] select-none shrink-0">$</span>
                  <OutputNameInput
        ...
        {/* Add condition row */}
        <button
          className="w-full flex items-center gap-1.5 px-2 py-1.5 text-[10px] text-[#333] hover:text-[#555] hover:bg-white/[0.02] rounded-md transition-colors cursor-pointer select-none"
          onClick={addRow}
        >
          <Plus size={10} />
                  ...
                  {detailedResult?.finalError && !selectedVarUsable ? (
                    ...
                    <span className="text-[10px] font-mono select-none shrink-0" style={{
                      ...
                      {resolvedSource === 'fallback' && (
                        <span className="text-[#444] ml-1">(fallback)</span>
                      )}
                    </span>
                  )}
                </div>
      ...
      >
        <span className="text-[9px] tracking-wide uppercase text-[#333] select-none">
          Fallback
        </span>
        <span className="text-[10px] font-mono text-[#444] select-none">
    ...
    if (ro.result.type === 'computedColor') {
      ...
      return (
        <span className="flex items-center gap-1 shrink-0">
          ...
          <span className="text-[9px] font-mono text-[#888] select-none">{displayStr}</span>
        </span>
      );
    }
    ...
    <div className="flex flex-1 min-h-0">
      ...
      <div className="flex flex-col shrink-0 p-3 overflow-y-auto" style={{ width: '15%', minWidth: 120, maxWidth: 200, borderRight: '1px solid rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.01)' }}>
        ...
        <div className="flex flex-col gap-2">
          ...
          {currentValueToken && (
            <div>
              <div className="text-[8px] text-[#555] uppercase tracking-wide mb-0.5 select-none">Current Value</div>
          ...
          {logic.rows.length > 0 && (
            <div>
              <div className="text-[8px] text-[#555] uppercase tracking-wide mb-0.5 select-none">Rows</div>
              <div className="text-[11px] font-mono text-[#888]">{logic.rows.length}</div>
            </div>
          )}
        </div>
      </div>
      ...
      >
        ...
        >
          <div className="flex items-center gap-1.5">
            <span className="text-[11px] tracking-wide uppercase text-[#555] select-none">Assign Token</span>
            {hasRows && (
              <button
                className="flex items-center justify-center w-4 h-4 rounded transition-colors cursor-pointer hover:bg-white/[0.08]"
                onClick={onPlay}
        ...
        {hasRows && (() => {
          ...
          const renderFinalValue = () => {
            if (hasAnyWarning || !resolvedResult) {
              // Fallback display
              return (
                <span className="flex items-center gap-1 text-[10px] font-mono text-[#555] select-none shrink-0">
                  {currentValueToken ? `{${currentValueToken}}` : '—'}
                  {resolvedSource === 'fallback' && currentValueToken && (
                    <span className="text-[#444]">(fallback)</span>
                  )}
                </span>
              );
            }
            ...
            return <span className="text-[10px] font-mono text-[#333] select-none">—</span>;
          };
        ...
        <div
          ...
          style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }}
        >
          <span className="text-[9px] tracking-wide uppercase text-[#333] select-none">Fallback</span>
          <span className="text-[10px] font-mono text-[#444] select-none">{currentValueToken ? `{${currentValueToken}}` : 'Manual assignment'}</span>
        </div>
      </div>
    </div>
  );
...
function NodeViewSection({ colorSpace, channelDefs, config, onUpdateConfig, readOnly, hasTokensAssigned = false }: NodeViewSectionProps) {
  ...
  return (
    <div>
      ...
      >
        {isExpanded ? <ChevronDown size={10} className="text-[#555]" /> : <ChevronRight size={10} className="text-[#555]" />}
        <span className="text-[10px] uppercase tracking-wider text-[#555] group-hover:text-[#888] transition-colors select-none">
          Node View
        </span>
        {activeCount > 0 && (
          <span
        ...
        <div className="mt-2 space-y-1.5">
          {allChannels.map(ch => {
            ...
            return (
              <div key={ch.key} className="space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-[#777] select-none">{ch.label}</span>
                ...
                {hasSlider && !isEditingThis && !isHidden && (
                  <div
                    className="flex items-center gap-1 pl-1 cursor-pointer group/sr"
          ...
          {!hasTokensAssigned && (
            <div className="space-y-1 mt-1.5 pt-1.5" style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }}>
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-[#777] select-none">Token Section</span>
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => {
                      if (readOnly) return;
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[13px] text-[#888] select-none">Advanced</span>
          <span
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            className="flex items-center justify-center w-6 h-6 rounded-md text-[#555] hover:text-[#ccc] hover:bg-white/[0.06] transition-colors cursor-pointer"
      ...
      {isMinimized ? (
        ...
        {isTokenNodeChild ? (
            ...
            <div className="flex-1 flex min-h-0">
              ...
              >
                <div className="px-4 pt-4 pb-3 flex-1 overflow-y-auto min-h-0">
                  ...
                  <div className="space-y-2">
                    {channelDefs.map((ch, i) => {
                      ...
                      return (
                        <div key={ch.key} className="flex items-center justify-between">
                          <span className="text-[11px] text-[#555] select-none">{ch.label}</span>
                          {isConditioned ? (
                            <span
                            ...
                            <span className="text-[11px] text-[#999] font-mono select-none">
                              {fmtVal(colorInfo.channels[i], ch.key)}
                              {getUnit(ch.key, colorInfo.colorSpace)}
                            </span>
                          )}
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="flex items-center justify-between">
                          <span className="text-[11px] text-[#555] select-none">Alpha</span>
                          {isAlphaConditioned ? (
                            <span
                              className="text-[10px] font-mono px-1.5 py-[1px] rounded select-none"
                              style={{
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] text-[#555] select-none">Hex</span>
                    <span className="text-[11px] text-[#999] font-mono select-none">
                      {colorInfo.hex}
                    </span>
                  </div>
                ...
                <div className="mt-auto px-4 pb-3 pt-2 shrink-0">
                  ...
                  >
                    <HelpCircle size={12} style={{ color: '#444' }} className="group-hover:!text-[#666] transition-colors" />
                    <span className="text-[10px] text-[#444] group-hover:text-[#666] transition-colors select-none">Reference Guide</span>
                  </button>
                </div>
              </div>
          ...
          <div className="flex-1 overflow-auto p-6">
            <div className="text-[13px] text-[#444] select-none text-center mt-12">
