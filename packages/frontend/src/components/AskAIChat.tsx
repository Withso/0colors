      ...
      {parts.map((part, i) => {
        if (part.startsWith('```') && part.endsWith('```')) {
          ...
          return (
            ...
            >
              <code className="text-[#ccc]">{code}</code>
            </pre>
          );
        }
        // Bold
        const bolded = part.replace(/\*\*(.*?)\*\*/g, '<b class="text-[#ddd] font-medium">$1</b>');
        // Inline code
        const coded = bolded.replace(/`([^`]+)`/g, '<code class="px-1 py-[1px] rounded text-[10.5px] text-[#ccc]" style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.05)">$1</code>');
...
const ERROR_ICON_COLORS: Record<string, string> = {
  'rate_limit_exceeded': '#E5A336',
  'rate_limited': '#E5A336',
  '429': '#E5A336',
  'authentication_error': '#E93D82',
  '401': '#E93D82',
  '403': '#E93D82',
  'not_found': '#7C66DC',
  '404': '#7C66DC',
  'network_error': '#E93D82',
  '0': '#E93D82',
};
...
export function AskAIChat({
  ...
  const headerLeft = (
    <div className="flex items-center gap-2">
      ...
      <button
        ...
        style={{ color: showSidebar ? '#ccc' : '#555' }}
      >
        <Menu size={14} />
      </button>
      <div className="flex items-center gap-1.5">
        <Sparkles size={13} className="text-[#E5A336]" />
  ...
  const headerRight = (
    <div className="flex items-center gap-1">
      ...
      <button
        ...
        className="p-1.5 rounded-md hover:bg-white/5 transition-colors cursor-pointer"
        style={{ color: isDocked ? '#E5A336' : '#555' }}
          ...
          {showSidebar ? (
              ...
              <div className="flex-1 overflow-y-auto">
                {conversations.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-center px-6">
                    ...
                    <p className="text-[12px] text-[#555] mb-1">No conversations yet</p>
                      ...
                      return (
                        ...
                        >
                          <div className="shrink-0 mt-0.5">
                            <MessageSquare size={12} style={{ color: isActive ? '#E5A336' : '#444' }} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2">
                              <span className="text-[12px] truncate" style={{ color: isActive ? '#ddd' : '#999' }}>
                                {conv.title}
                              </span>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-[9px]" style={{ color: msgCount >= MAX_MESSAGES_PER_CONVERSATION ? '#E5A336' : '#333' }}
            ...
            >
              ...
              <div ref={messagesContainerRef} className="flex-1 overflow-y-auto px-3 py-3 space-y-3 relative">
                {!hasAccess ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center px-4">
                    ...
                    <p className="text-[13px] text-[#666] mb-1">Ask AI is available for Cloud and Template projects</p>
                    <p className="text-[10px] text-[#444]">Switch to a Cloud project or open a Template to use Ask AI.</p>
                  </div>
                ) : !activeConversation || activeConversation.messages.length === 0 ? (
                  /* ── Empty state ── */
                  <div className="flex flex-col items-center justify-center h-full text-center px-4">
                    <Sparkles size={28} className="text-[#E5A336]/30 mb-3" />
                    ...
                    <p className="text-[9px] text-[#333] mt-3 leading-relaxed max-w-[260px]">
                      The AI has full context of your current project — nodes, tokens, themes, logic, and more.
                    </p>
                    {!isProviderConfigured(aiSettings) && (
                      <button
                        onClick={() => setShowSettings(true)}
                        className="mt-3 text-[10px] text-[#E5A336] hover:underline cursor-pointer"
                      >
                  ...
                  <>
                      ...
                      <div className="flex justify-start">
                        ...
                        >
                          <div className="text-[12px] leading-[1.6] text-[#aaa] whitespace-pre-wrap break-words">
                            ...
                            <span className="inline-block w-1.5 h-3.5 bg-[#E5A336] ml-0.5 animate-pulse rounded-sm" />
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {isStreaming && !streamingText && (
                      <div className="flex justify-start">
                        ...
                        >
                          <div className="flex items-center gap-1">
                            <div className="w-1.5 h-1.5 rounded-full bg-[#E5A336] animate-bounce" style={{ animationDelay: '0ms' }} />
                            <div className="w-1.5 h-1.5 rounded-full bg-[#E5A336] animate-bounce" style={{ animationDelay: '150ms' }} />
                            <div className="w-1.5 h-1.5 rounded-full bg-[#E5A336] animate-bounce" style={{ animationDelay: '300ms' }} />
                          </div>
                        </div>
                      </div>
                    )}
              ...
              {hasAccess && activeConversation && activeConversation.messages.length >= MAX_MESSAGES_PER_CONVERSATION && !isStreaming && (
                ...
                >
                  <MessageSquare size={13} style={{ color: '#E5A336', flexShrink: 0 }} />
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] text-[#E5A336] leading-tight">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button
                      ...
                      style={{ background: 'rgba(229,163,54,0.1)', color: '#E5A336', border: '1px solid rgba(229,163,54,0.15)' }}
                    ...
                    <button
                      onClick={startNewConversation}
                      className="flex items-center gap-1 h-6 px-2 rounded-md text-[9px] cursor-pointer transition-colors"
                      style={{ background: 'rgba(229,163,54,0.15)', color: '#E5A336', border: '1px solid rgba(229,163,54,0.2)' }}
                    >
                      <Plus size={9} />
                      New Chat
                    </button>
                ...
                <div className="shrink-0 px-3 pb-3 pt-1">
                  ...
                  >
                    <textarea
                      ...
                      className="w-full px-3 pt-2.5 pb-1 text-[12px] text-[#ccc] placeholder:text-[#333] bg-transparent outline-none resize-none"
                      style={{ minHeight: TEXTAREA_MIN_H, maxHeight: TEXTAREA_MAX_H }}
                    />
                    <div className="flex items-center justify-between px-2.5 pb-2">
                      <div className="flex items-center gap-2">
                        {charCount > 500 && (
                          <span className={`text-[9px] ${isOverLimit ? 'text-[#E93D82]' : 'text-[#333]'}`}>
                      ...
                      <div className="flex items-center gap-1">
                        {isStreaming ? (
                          <button
                            ...
                            style={{ background: 'rgba(233,61,130,0.15)', color: '#E93D82', border: '1px solid rgba(233,61,130,0.2)' }}
                          >
                            <StopCircle size={12} />
                            Stop
                          </button>
                          ...
                          <button
                            ...
                            style={{
                              background: input.trim() && !isOverLimit ? '#E5A336' : 'rgba(255,255,255,0.04)',
  ...
  return createPortal(
    ...
    >
      ...
      >
        <div className="w-8 h-1 mx-auto mt-1 rounded-full bg-[#333] group-hover:bg-[#555] transition-colors" />
      </div>
