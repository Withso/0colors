...
function TokenName({ name, onDoubleClick, panelWidth, hasCheckbox }: { name: string; onDoubleClick: () => void; panelWidth: number; hasCheckbox: boolean }) {
  ...
  return (
    <div
      className="text-xs text-[#ededed] cursor-default overflow-hidden whitespace-nowrap px-2 flex items-center gap-1"
      ...
      <div className="min-w-[190px] max-w-[300px]">
        ...
        <div className="px-3 pt-1.5 pb-1.5">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="text-[10px] font-mono text-[#888] truncate">--{name.toLowerCase().replace(/\s+/g, '-')}</span>
          </div>
        </div>
            ...
            <div className="px-3 pt-2 pb-1.5">
              ...
              <div className="flex items-center gap-2">
                {valueTokenRef.isChainRef ? (
                  ...
                  <div className="w-3 h-3 rounded-[3px] shrink-0 border border-[#222]/50 opacity-40" />
                )}
                <span className="text-[11px] text-[#ccc] truncate">{valueTokenRef.name}</span>
              </div>
            </div>
                ...
                <div className="px-3 pt-1.5 pb-2.5 space-y-1">
                  {valueTokenRef.spaceName && valueTokenRef.spaceValue && (
                    <div className="flex items-center justify-between gap-4">
                      <span className="text-[10px] text-[#666] uppercase tracking-wide">{valueTokenRef.spaceName}</span>
                      <span className="text-[10px] font-mono text-[#888]">{valueTokenRef.spaceValue}</span>
                    </div>
                  )}
                  {valueTokenRef.hex && (
                    <div className="flex items-center justify-between gap-4">
                      <span className="text-[10px] text-[#666] uppercase tracking-wide">HEX</span>
                      <span className="text-[10px] font-mono text-[#888]">{valueTokenRef.hex}</span>
                    </div>
                  )}
                  {valueTokenRef.alpha !== undefined && valueTokenRef.alpha < 100 && (
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <span className="text-[10px] font-mono text-[#888]">{Math.round(valueTokenRef.alpha)}%</span>
                    </div>
                  )}
                </div>
              </>
    ...
    <div className="min-w-[180px] max-w-[280px]">
      <div className="flex items-center gap-2 px-3 pt-2.5 pb-2">
        {color ? (
          ...
          <div className="w-3.5 h-3.5 rounded-[4px] shrink-0 border border-dashed border-[#222]/50 opacity-40" />
        )}
        <span className="text-[11px] text-[#e0e0e0] truncate">{name}</span>
      </div>
      {isEmpty && !hasColorInfo && (
        <>
          <div className="h-px bg-[#ffffff]/[0.06] mx-2" />
          <div className="px-3 pt-2 pb-2.5">
            <span className="text-[10px] text-[#555] italic">Empty — no color values assigned</span>
          </div>
        </>
      )}
      {hasColorInfo && (
        <>
          ...
          <div className="px-3 pt-2 pb-2.5 space-y-1">
            <div className="flex items-center justify-between gap-4">
              <span className="text-[10px] text-[#666] uppercase tracking-wide">{spaceName}</span>
            ...
            {showAlpha && (
              <div className="flex items-center justify-between gap-4">
                <span className="text-[10px] text-[#666] uppercase tracking-wide">ALPHA</span>
                <span className="text-[10px] font-mono text-[#888]">{Math.round(alpha)}%</span>
              </div>
            )}
          </div>
        </>
...
export function TokensPanel({ tokens, nodes, allProjectTokens = [], allProjectNodes = [], projects, pages, groups, activeProjectId, activePageId, activeThemeId, isPrimaryTheme = true, primaryThemeId, showAllVisible = false, onAddToken, onUpdateToken, onDeleteToken, onUpdateProjects, onUpdatePages, onUpdateGroups, onExportProject, onImportProject, onUpdateNode, onDeleteNode, onNavigateToNode, onNavigateToProjects, advancedLogic, cloudSyncStatus = 'local', lastSyncedAt, lastSyncError, onManualSync, dirtyCount = 0, readOnly = false }: TokensPanelProps) {
  ...
  const renderDropIndicator = () => (
    <div className="h-[2px] bg-[#465BFE] rounded-full mx-2 my-[1px] pointer-events-none" />
  );
  // Drop indicator line for group-level reordering
  const renderGroupDropIndicator = () => (
    <div className="h-[2px] bg-[#465BFE] rounded-full mx-1 my-[2px] pointer-events-none" />
  );
      ...
      >
        <ContextMenuTrigger asChild>
          <div
            ...
            className={`group/token ${isSelected ? 'rounded-r-md' : 'rounded-md'} transition-all overflow-hidden w-full relative ${reorderDropIndicator?.dragTokenIds.includes(token.id) ? '!opacity-40' : ''
                ...
                ? 'ring-1 ring-[#465BFE]/60 bg-[#465BFE]/[0.12]'
                : isSelected
                  ? 'bg-[#465BFE]/[0.08]'
                  : contextMenuOpenTokenId === token.id
                    ? 'bg-[#ffffff]/[0.04]'
                    : 'hover:bg-[#ffffff]/[0.03] cursor-pointer'
              } ${inherited && !showAllVisible ? 'opacity-[0.55] hover:opacity-100' : ''} ${tokenIsHidden ? 'opacity-[0.4] hover:opacity-70' : ''} ${tokenNodeHiddenOnCanvas && !tokenIsHidden ? 'opacity-[0.4] hover:opacity-70' : ''} ${focusedTokenId === token.id && isSelected ? 'ring-1 ring-[#465BFE]/40' : ''}`}
            onClick={(e) => {
              const target = e.target as HTMLElement;
              if (
                target.closest('input') ||
          ...
          >
            {isSelected && <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-[#465BFE]" />}
            <Tooltip delayDuration={400} open={editingId === token.id ? false : undefined}>
                ...
                <div className="relative flex items-center gap-2 w-full min-w-0 overflow-hidden px-2.5 py-[7px]">
                  ...
                  {!isPrimaryTheme && (
                    <div className="shrink-0 -ml-1 mr-[-4px] w-3 flex items-center justify-center">
                      {isTokenValueChanged(token.id) && (
                        <Crown className="h-2.5 w-2.5 text-[#465BFE] fill-[#465BFE]" />
                      )}
                    </div>
                  )}
                  {isTokenNodeToken ? (
                    valueTokenInfo?.color && valueTokenInfo.color !== 'transparent' ? (
                      <div
                        className={`w-3.5 h-3.5 rounded-[3px] shrink-0 transition-shadow ${isSelected ? 'ring-1.5 ring-[#465BFE]' : ''}`}
                        style={{ backgroundColor: valueTokenInfo.color }}
                      />
                    ) : (
                      <div
                        className={`w-3.5 h-3.5 rounded-[3px] shrink-0 border border-dashed border-[#222]/50 opacity-40 ${isSelected ? 'ring-1.5 ring-[#465BFE]' : ''}`}
                      />
                    )
                  ) : (
                    <div
                      className={`w-3.5 h-3.5 rounded-[3px] shrink-0 transition-shadow ${(nodesUsingToken.length === 0 || isEmptyToken) ? 'border border-dashed border-[#222]/50' : ''} ${isSelected ? 'ring-1.5 ring-[#465BFE]' : ''}`}
                  ...
                  {editingId === token.id && editingType === 'token' && !isTokenNodeToken ? (
                    <input
                      ...
                      className="bg-transparent border-none text-[#ededed] px-2 py-0 m-0 min-w-0 flex-1 outline-none caret-[#465BFE] select-text"
                  ...
                  {isTokenNodeToken ? (
                    <div className="flex items-center gap-[3px] ml-auto shrink-0">
                      ...
                      {tokensWithActiveLogic.has(token.id) ? (
                        ...
                        <span className="text-[10px] font-mono text-[#555] px-1 py-px rounded bg-[#ffffff]/[0.04] shrink-0">
                          TOKEN
                        </span>
                      ) : null}
                    ...
                    const badgeTextColor = nodeInheritanceState === 'modified' ? 'text-[#465BFE]'
                      : nodeInheritanceState === 'inherited' ? 'text-[#d4a017]'
                        : 'text-[#555]';
                    const badgeBgColor = nodeInheritanceState === 'modified' ? 'bg-[#465BFE]/[0.12]'
                      : nodeInheritanceState === 'inherited' ? 'bg-[#FBBF24]/[0.10]'
                        : 'bg-[#ffffff]/[0.04]';
                    ...
                    return (
                      <div className="flex items-center gap-[3px] ml-auto shrink-0">
                        ...
                        {nodeType && nodeType === 'spacing' ? (
                          <>
                            <span className="text-xs font-mono text-[#555] shrink-0">
              ...
              <TooltipPrimitive.Portal>
                <TooltipPrimitive.Content
                  side="right"
                  sideOffset={8}
                  className="z-50 bg-[#161616]/95 backdrop-blur-md border border-[#ffffff]/[0.06] rounded-lg shadow-[0_4px_24px_rgba(0,0,0,0.5)] animate-in fade-in-0 zoom-in-95 data-[side=right]:slide-in-from-left-2 data-[side=left]:slide-in-from-right-2"
                >
                  <TokenTooltipBody
                    name={token.name}
        ...
        <ContextMenuContent className="bg-[#0e0e0e] border border-[#141414] rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.5)] p-1 min-w-[160px]">
          ...
          {selectionHasMixedTokenNodeTypes && isSelected ? (
            <ContextMenuItem
              disabled
              className="text-[#555] cursor-default rounded-lg px-2.5 py-2 text-xs gap-2"
            >
            ...
            <>
              ...
              >
                <Check className={`h-3.5 w-3.5 ${isSelected ? 'text-[#ededed]' : 'text-[#555]'}`} />
              ...
              {tokenNodeOnCanvas && onNavigateToNode && tokenNodeOnCanvas.pageId === activePageId && (
                <ContextMenuItem
                  ...
                  className="text-[#ededed] focus:bg-[#ffffff]/[0.06] focus:text-[#ededed] cursor-pointer rounded-lg px-2.5 py-2 text-xs gap-2"
                >
                  <Target className="h-3.5 w-3.5 text-[#555]" />
                  Navigate to node
                </ContextMenuItem>
              )}
              {isPrimaryTheme && (
                <>
                  ...
                  <ContextMenuItem
                    ...
                    className="text-[#ededed] focus:bg-[#ffffff]/[0.06] focus:text-[#ededed] cursor-pointer rounded-lg px-2.5 py-2 text-xs gap-2"
                  >
                    <ArrowUp className="h-3.5 w-3.5 text-[#555]" />
                    Move up
                  </ContextMenuItem>
                  <ContextMenuItem
                    onClick={() => {
                    ...
                    className="text-[#ededed] focus:bg-[#ffffff]/[0.06] focus:text-[#ededed] cursor-pointer rounded-lg px-2.5 py-2 text-xs gap-2"
                  >
                    <ArrowDown className="h-3.5 w-3.5 text-[#555]" />
                    Move down
                  </ContextMenuItem>
                </>
              )}
              ...
              {isPrimaryTheme && !isReadOnly && (
                <>
                  <ContextMenuItem
                    ...
                    className="text-[#ededed] focus:bg-[#ffffff]/[0.06] focus:text-[#ededed] cursor-pointer rounded-lg px-2.5 py-2 text-xs gap-2"
                  >
                    <Edit2 className="h-3.5 w-3.5 text-[#555]" />
                    Rename
                  </ContextMenuItem>
                  <ContextMenuSeparator className="bg-[#1e1e1e] my-1" />
                  <ContextMenuItem
                  ...
                  <ContextMenuSeparator className="bg-[#1e1e1e] my-1" />
                  <ContextMenuSub>
                    <ContextMenuSubTrigger className="text-[#ededed] focus:bg-[#ffffff]/[0.06] focus:text-[#ededed] data-[state=open]:bg-[#ffffff]/[0.06] cursor-pointer rounded-lg px-2.5 py-2 text-xs gap-2">
                      <Folder className="h-3.5 w-3.5 text-[#555]" />
                    ...
                    <ContextMenuSubContent className="bg-[#0e0e0e] border border-[#141414] rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.5)] p-1 min-w-[140px] max-h-[300px] overflow-y-auto">
                      <ContextMenuItem
                        ...
                        className="text-[#ededed] focus:bg-[#ffffff]/[0.06] focus:text-[#ededed] cursor-pointer rounded-lg px-2.5 py-2 text-xs"
                      >
                        No group
                      </ContextMenuItem>
                      ...
                      {regularGroups.map(g => (
                        <ContextMenuItem
                          ...
                          className="text-[#ededed] focus:bg-[#ffffff]/[0.06] focus:text-[#ededed] cursor-pointer rounded-lg px-2.5 py-2 text-xs gap-2"
                        >
                          <Folder className="h-3 w-3 text-[#555]" />
                          {g.name}
                        </ContextMenuItem>
                      ))}
                      <ContextMenuSeparator className="bg-[#1e1e1e] my-1" />
                      <ContextMenuItem
                        ...
                        className="text-[#ededed] focus:bg-[#ffffff]/[0.06] focus:text-[#ededed] cursor-pointer rounded-lg px-2.5 py-2 text-xs gap-2"
                      >
                        <FolderPlus className="h-3 w-3 text-[#555]" />
                        New group
                      </ContextMenuItem>
                    </ContextMenuSubContent>
                  </ContextMenuSub>
                  ...
                  <ContextMenuItem
                    ...
                    className="text-[#FF4D6A] focus:bg-[#ffffff]/[0.06] focus:text-[#FF7A90] cursor-pointer rounded-lg px-2.5 py-2 text-xs gap-2"
                  >
  ...
  return (
    <TooltipProvider delayDuration={300}>
      ...
      >
        {/* Projects Island */}
        <div className="shrink-0 rounded-2xl px-3 h-14 flex items-center w-full min-w-0" style={{ backgroundColor: '#111111' }}>
          <div className="flex items-center justify-between w-full min-w-0">
            <h2 className="flex items-center gap-2 text-white text-sm min-w-0 flex-1 truncate">
              <Tip label="All Projects" side="bottom">
                <Home
                  className="h-4 w-4 shrink-0 cursor-pointer hover:text-[#ededed] transition-colors"
              ...
              {isEditingProjectName ? (
                <input
                  ...
                  className="bg-[#1a1a1a] text-white text-sm px-2 py-0.5 rounded border border-transparent focus:border-[#465BFE] focus:outline-none min-w-0 flex-1"
                />
              ) : (
                <span
                  className={`truncate ${isReadOnly ? 'cursor-default' : 'cursor-text hover:text-[#ededed]'} transition-colors`}
        ...
        {/* Tokens Island */}
        <div className="flex-1 flex flex-col overflow-hidden rounded-2xl min-h-0 relative" style={{ backgroundColor: '#111111' }}>
          {/* Search + Filters */}
          ...
          <div className="px-2 pb-1.5 shrink-0 flex gap-1">
            {isSearchOrFilterActive ? (
              ...
              <div className="px-2.5 py-[3px] rounded-md text-[11px] flex items-center gap-1.5 bg-[#ffffff]/[0.08] text-[#ededed]">
                All Results
              ...
              <>
                <button
                  ...
                  className={`px-2.5 py-[3px] rounded-md text-[11px] transition-colors flex items-center gap-1.5 ${viewFilter === 'tokens'
                      ? 'bg-[#ffffff]/[0.08] text-[#ededed]'
                      : 'text-[#555] hover:text-[#888] hover:bg-[#ffffff]/[0.03]'
                    }`}
                >
                  Tokens
                  {(filteredRegularGroups.length + filteredUngroupedTokens.length) > 0 && (
                    <span className={`text-[10px] ${viewFilter === 'tokens' ? 'text-[#888]' : 'text-[#444]'}`}>
                ...
                >
                  ...
                  {filteredPaletteEntries.length > 0 && (
                    <span className={`text-[10px] ${viewFilter === 'palettes' ? 'text-[#888]' : 'text-[#444]'}`}>
                      ...
                      <div className="flex items-center gap-1 group/project w-full min-w-0 pl-0.5 pr-1">
                        ...
                        {((editingId === activeProject.id && editingType === 'project') || (editingId === activePage?.id && editingType === 'page')) ? (
                          <Input
                            ...
                            onBlur={saveName}
                            onClick={(e) => e.stopPropagation()}
                            className="h-6 text-xs min-w-0 flex-1 bg-[#1a1a1a] border-transparent text-[#ededed] select-text"
                          />
                        ) : (
                          <div className="flex-1 min-w-0 overflow-hidden">
                            <span
                              className={`text-xs text-[#888] ${isReadOnly ? 'cursor-default' : 'cursor-text hover:text-[#ededed]'} transition-colors px-1 py-0.5 rounded ${isReadOnly ? '' : 'hover:bg-[#ffffff]/[0.04]'} block truncate`}
                      ...
                      <>
                        ...
                        {unifiedItems.map((item, itemIdx) => {
                          if (item.type === 'paletteEntry') {
                            ...
                            const groupBadgeTextA = anyShadeModifiedA ? 'text-[#465BFE]' : isPalInherited ? 'text-[#FBBF24]' : 'text-[#555]';
                            const groupBadgeBgA = anyShadeModifiedA ? 'bg-[#465BFE]/[0.12]' : isPalInherited ? 'bg-[#FBBF24]/[0.12]' : 'bg-[#ffffff]/[0.04]';
                                ...
                                <ContextMenu>
                                  <ContextMenuTrigger asChild>
                                    <div
                                      ...
                                      className="flex items-center gap-1.5 px-1.5 py-1.5 rounded-md hover:bg-[#ffffff]/[0.03] transition-colors w-full min-w-0 group outline-none focus-visible:ring-1 focus-visible:ring-[#465BFE]/50"
                                    >
                                      {!isReadOnly && isPrimaryTheme && (
                                        ...
                                        >
                                          <GripVertical className="h-3 w-3 text-[#555]" />
                                        </div>
                                      )}
                                      {!isPrimaryTheme && (
                                      ...
                                      <div className="flex-1 min-w-0 flex flex-col gap-1">
                                        {editingId === paletteNode.id && editingType === 'group' ? (
                                          <Input
                                            ...
                                            onClick={(e) => e.stopPropagation()}
                                            className="h-auto text-xs bg-[#1a1a1a] border-transparent text-[#ededed] px-2 py-0 min-w-[120px] flex-1 select-text"
                                          />
                                        ) : (
                                          <div className="flex items-center gap-1 w-full">
                                            <div className="flex items-center gap-1.5 flex-1 min-w-0">
                                              <span
                                                className={`text-xs text-[#888] truncate ${isReadOnly ? 'cursor-default' : 'cursor-text hover:text-[#ededed]'} transition-colors`}
                                              ...
                                              {isPalHiddenA && (
                                                <EyeOff className="h-2.5 w-2.5 text-[#555] shrink-0" />
                                              )}
                                            </div>
                                            {!isReadOnly && (
                                              <Tip label={paletteNode.paletteNameLocked ? "Unlock Palette Name" : "Lock Palette Name"} side="bottom">
                                                ...
                                                >
                                                  {paletteNode.paletteNameLocked ? (
                                                    <Lock className="w-3 h-3 text-[#465BFE]" />
                                                  ) : (
                                            ...
                                            {!isReadOnly && (
                                              <Tip label="Delete Palette" side="bottom">
                                                <button
                                                  ...
                                                  className="w-5 h-5 rounded transition-colors flex items-center justify-center hover:bg-[#ffffff]/[0.06] shrink-0 opacity-0 group-hover:opacity-100"
                                                >
                                                  <Trash2 className="w-3 h-3 text-[#555] hover:text-[#FF4D6A]" />
                                                </button>
                                              </Tip>
                                            )}
                                          </div>
                                    ...
                                    {!isReadOnly && (
                                      <>
                                        ...
                                        <ContextMenuItem
                                          ...
                                          className="text-[#FF4D6A] focus:bg-[#ffffff]/[0.06] focus:text-[#FF4D6A] cursor-pointer rounded-lg px-2.5 py-2 text-xs gap-2"
                                        >
                                          <Trash2 className="h-3.5 w-3.5" />
                                          Delete palette
                                        </ContextMenuItem>
                                          ...
                                          <TooltipTrigger asChild>
                                            <div
                                              ...
                                              className={`flex items-center gap-2 px-2.5 py-[6px] ${shadeIsSelected1 ? 'rounded-r-md' : 'rounded-md'} transition-all cursor-pointer relative ${highlightedTokenId === shadeToken.id
                                                ...
                                                } ${isTokenInherited(shadeToken.id) && !showAllVisible ? 'opacity-[0.55] hover:opacity-100' : ''} ${focusedTokenId === shadeToken.id && shadeIsSelected1 ? 'ring-1 ring-[#465BFE]/40' : ''}`}
                                              onClick={(e) => {
                                                const target = e.target as HTMLElement;
                                            ...
                                            >
                                              {shadeIsSelected1 && <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-[#465BFE]" />}
                                              <div
                                                className={`w-3.5 h-3.5 rounded-[3px] shrink-0 transition-shadow ${shadeIsSelected1 ? 'ring-1.5 ring-[#465BFE]' : ''}`}
                                              ...
                                              {editingId === shadeToken.id && editingType === 'token' ? (
                                                <input
                                                  ...
                                                  onClick={(e) => e.stopPropagation()}
                                                  onDoubleClick={(e) => e.stopPropagation()}
                                                  className="bg-transparent border-none text-[#ededed] px-0 py-0 m-0 min-w-0 flex-1 outline-none caret-[#465BFE] select-text"
                                                  style={{ fontSize: '12px', lineHeight: 'normal', height: 'auto' }}
                                                  maxLength={MAX_TOKEN_NAME}
                                                />
                                              ) : (
                                                <span className="text-xs text-[#ededed] truncate flex-1 min-w-0" title={shadeToken.name}>
                                                  {shadeToken.name}
                                                </span>
                                              )}
                                            </div>
                          ...
                          if (isPaletteEntry && group.paletteNodeId) {
                            ...
                            const groupBadgeTextB = anyShadeModifiedB ? 'text-[#465BFE]' : isPaletteInherited ? 'text-[#FBBF24]' : 'text-[#555]';
                            const groupBadgeBgB = anyShadeModifiedB ? 'bg-[#465BFE]/[0.12]' : isPaletteInherited ? 'bg-[#FBBF24]/[0.12]' : 'bg-[#ffffff]/[0.04]';
                                      ...
                                      return (
                                        <Tooltip key={shade.id} delayDuration={400} open={editingId === shadeToken.id ? false : undefined}>
                                          <TooltipTrigger asChild>
                                            <div
                                              ...
                                              className={`flex items-center gap-2 px-2.5 py-[6px] ${shadeIsSelected2 ? 'rounded-r-md' : 'rounded-md'} transition-all cursor-pointer relative ${highlightedTokenId === shadeToken.id
                                                  ...
                                                  : shadeIsSelected2
                                                    ? 'bg-[#465BFE]/[0.08]'
                                                    : 'hover:bg-[#ffffff]/[0.03]'
                                                } ${isTokenInherited(shadeToken.id) && !showAllVisible ? 'opacity-[0.55] hover:opacity-100' : ''} ${focusedTokenId === shadeToken.id && shadeIsSelected2 ? 'ring-1 ring-[#465BFE]/40' : ''}`}
                                              onClick={(e) => {
                                                const target = e.target as HTMLElement;
                                                if (target.closest('input')) return;
                                                e.stopPropagation();
                                            ...
                                            >
                                              {shadeIsSelected2 && <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-[#465BFE]" />}
                                              <div
                                                className={`w-3.5 h-3.5 rounded-[3px] shrink-0 transition-shadow ${shadeIsSelected2 ? 'ring-1.5 ring-[#465BFE]' : ''}`}
                                                style={{ backgroundColor: shadeColor }}
                                              />
                                              {editingId === shadeToken.id && editingType === 'token' ? (
                                                <input
                              ...
                              <ContextMenu>
                                <ContextMenuTrigger asChild>
                                  <div
                                    ...
                                    className="flex items-center gap-1 group/group w-full min-w-0 py-1.5 pl-0.5 pr-1 outline-none focus-visible:ring-1 focus-visible:ring-[#465BFE]/50 rounded"
                                  >
                                    {isPrimaryTheme && (
                                      <div
                                    ...
                                    {editingId === group.id && editingType === 'group' && !isTokenNodeGrp ? (
                                      ...
                                      <div className="flex-1 min-w-0 overflow-hidden">
                                        <span
                                          className={`text-xs text-[#888] ${isReadOnly || isTokenNodeGrp ? 'cursor-default' : 'cursor-text hover:text-[#ededed]'} transition-colors px-1 py-0.5 rounded ${isReadOnly || isTokenNodeGrp ? '' : 'hover:bg-[#ffffff]/[0.04]'} block truncate`}
                                    ...
                                    {isPrimaryTheme && !isReadOnly && !isTokenNodeGrp && (
                                      <>
                                        <Tip label="Delete Group" side="bottom">
                                          ...
                                          >
                                            <Trash2 className="h-3 w-3 text-[#555] hover:text-[#FF4D6A]" />
                                          </button>
                                        </Tip>
                                ...
                                <div data-group-zone={!isColorPaletteGroup ? group.id : undefined} className="space-y-px w-full min-w-0 mt-0.5">
                                  {isColorPaletteGroup ? (
                                    ...
                                    paletteEntries.length === 0 ? (
                                      <div className="text-xs text-[#555] py-1.5 px-3">
                                        No palettes yet
                                      </div>
                                    ) : (
                                      paletteEntries.map(entry => {
                                        ...
                                        const groupBadgeTextC = anyShadeModifiedC ? 'text-[#465BFE]' : isPalEntryInherited ? 'text-[#FBBF24]' : 'text-[#555]';
                                          ...
                                          <div key={entry.id} className={`mb-0.5 min-w-0 w-full ${isPalHiddenC ? 'opacity-[0.4]' : ''}`} style={palEntryInheritedStyle}>
                                            <div className="flex items-center gap-1.5 py-1.5 rounded-md hover:bg-[#ffffff]/[0.03] transition-colors w-full min-w-0 group">
                                              ...
                                              <div className="flex-1 min-w-0 flex flex-col gap-1">
                                                <div className="flex items-center gap-1 w-full">
                                                  <div className="flex items-center gap-1.5 flex-1 min-w-0">
                                                    <span className="text-xs text-[#888] truncate">{paletteNode.paletteName || entry.name}</span>
                                                      ...
                                                      <TooltipTrigger asChild>
                                                        <div
                                                          ...
                                                          className={`flex items-center gap-2 px-2.5 py-[6px] ${selectedTokens.has(shadeToken.id) ? 'rounded-r-md' : 'rounded-md'} transition-all cursor-pointer relative ${selectedTokens.has(shadeToken.id)
                                                              ? 'bg-[#465BFE]/[0.08]'
                                                              : 'hover:bg-[#ffffff]/[0.03]'
                                                            } ${isTokenInherited(shadeToken.id) && !showAllVisible ? 'opacity-[0.55] hover:opacity-100' : ''} ${focusedTokenId === shadeToken.id && selectedTokens.has(shadeToken.id) ? 'ring-1 ring-[#465BFE]/40' : ''}`}
                                                          onClick={(e) => {
                                                            e.stopPropagation();
                                                            if (wasDraggingRef.current) return;
                                                            handleTokenClick(shadeToken.id, e);
                                                        ...
                                                        >
                                                          {selectedTokens.has(shadeToken.id) && <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-[#465BFE]" />}
                                                          <div
                                                            className={`w-3.5 h-3.5 rounded-[3px] shrink-0 transition-shadow ${selectedTokens.has(shadeToken.id) ? 'ring-1.5 ring-[#465BFE]' : ''}`}
                                                            style={{ backgroundColor: shadeColor }}
                                                          />
                                    ...
                                    <div className="text-xs text-[#555] py-1 px-3">
            ...
            <div className="border-t border-[#141414] bg-[#111111] shrink-0">
              ...
              <div className={`px-3 py-2 ${!isReadOnly ? 'border-b border-[#141414]' : ''} flex items-center justify-between`}>
                <div className="flex items-center gap-3">
                  <span className="text-xs text-[#465BFE]">{selectedTokens.size} variable{selectedTokens.size > 1 ? 's' : ''} selected</span>
                  <Button
                    size="sm"
                    variant="ghost"
                    className="h-7 px-3 text-xs text-[#888] hover:text-[#ededed] hover:bg-[#ffffff]/[0.04]"
                    onClick={() => {
                ...
                <div className="flex items-center gap-1">
                  ...
                  <Tip label="Clear Selection" side="bottom">
                    <Button
                      size="sm"
                      variant="ghost"
                      className="h-6 w-6 p-0 text-[#555] hover:text-[#ededed] hover:bg-[#ffffff]/[0.06]"
                      onClick={(e) => {
                        e.stopPropagation();
                        deselectAllTokens();
                      }}
                ...
                <div className="px-3 py-2.5">
                  ...
                  {selectionHasMixedTokenNodeTypes ? (
                    <div className="flex items-center justify-center">
                      <span className="text-[10px] text-[#555]">Mixed selection — actions disabled</span>
                    </div>
                  ) : selectionAllTokenNodeTokens ? (
                    <div className="flex items-center justify-center">
                      <span className="text-[10px] text-[#555]">Token node variables — managed on canvas</span>
                    </div>
                  ) : (
                    <div className="flex items-center justify-center gap-2">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            size="sm"
                            variant="outline"
                            className="h-8 px-3 text-xs bg-[#1a1a1a] border-transparent hover:bg-[#222] text-[#ededed]"
                          >
                        ...
                        <DropdownMenuContent align="end" className="bg-[#161616] border-transparent max-h-[300px] overflow-y-auto">
                          <DropdownMenuItem
                            onClick={() => bulkMoveToGroup(null)}
                            className="text-[#ededed] focus:bg-[#ffffff]/[0.06] focus:text-[#ededed] cursor-pointer"
                          >
                            No group
                          </DropdownMenuItem>
                          ...
                          <Tip label={allTokensVisible ? 'Hide selected' : allTokensHidden ? 'Show selected' : 'Mixed visibility'} side="top">
                            <Button
                              ...
                              className={`h-8 w-8 p-0 text-xs border-transparent ${mixedTokenVis
                                  ...
                                  : allTokensHidden
                                    ? 'text-[#465BFE] bg-[#465BFE]/10 border-[#465BFE]/20 hover:bg-[#465BFE]/15'
                      ...
                      <Tip label="Delete selected" side="top">
                        <Button
                          size="sm"
                          variant="outline"
                          className="h-8 w-8 p-0 text-xs bg-[#FF4D6A]/10 border-[#FF4D6A]/20 hover:bg-[#FF4D6A]/15 text-[#FF4D6A] hover:text-[#FF7A90]"
      ...
      <AlertDialog open={deleteGroupDialogOpen} onOpenChange={setDeleteGroupDialogOpen}>
        <AlertDialogContent className="bg-[#111111] border-[#222]">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-[#ededed]">Delete Group?</AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="text-[#a1a1a1] space-y-2">
                {groupToDelete && (
                  <>
                    <div>
                      You are about to delete the group <span className="font-semibold text-[#ededed]">"{groupToDelete.name}"</span>.
                    </div>
                    <div>
                      This group contains <span className="font-semibold text-[#ededed]">{groupToDelete.tokenCount} token{groupToDelete.tokenCount !== 1 ? 's' : ''}</span>.
                    </div>
                    ...
                    <div className="mt-3 font-semibold text-[#FF4D6A]">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="bg-[#1a1a1a] border-transparent text-[#ededed] hover:bg-[#222]">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
        ...
        <AlertDialogContent className="bg-[#111111] border-[#222] max-w-[400px]">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-[#ededed] flex items-center gap-2">
              <Zap size={14} className="text-[#465BFE]" />
            ...
            <AlertDialogDescription asChild>
              <div className="text-[#a1a1a1] space-y-3">
                {autoAssignDeleteDialog.items.length === 1 ? (
                  <>
                    <div>
                      The token <span className="text-[#ededed] font-mono text-[12px] bg-[#1a1a1a] px-1.5 py-0.5 rounded">
                        {autoAssignDeleteDialog.items[0]?.tokenName}
                      </span> will be permanently deleted from the token panel.
                    </div>
                    <div className="text-[12px] text-[#888]">
                      This token was auto-assigned to node <span className="text-[#ccc]">{autoAssignDeleteDialog.items[0]?.nodeName}</span>.
                ...
                <label className="flex items-start gap-2.5 cursor-pointer group bg-[#1a1a1a] rounded-lg px-3 py-2.5 border border-transparent hover:border-[#222] transition-colors">
                  <div
                  ...
                  >
                    <div className="text-[12px] text-[#ededed] select-none">
                      Don&apos;t auto-assign {autoAssignDeleteDialog.items.length === 1 ? 'token for this node' : 'tokens for these nodes'}
                    </div>
                    <div className="text-[11px] text-[#666] select-none mt-0.5">
