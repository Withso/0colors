...
This folder holds the **human test catalog**, **automation metadata**, **generated raw reports**, and (optionally) **legacy / template Playwright projects**. **Most runnable E2E code** for the app lives under `packages/frontend/tests/e2e/`.
...
| **`reports/e2e-results.json`** | **Generated** (gitignored) | Playwright JSON reporter output (see repo root `playwright.config.ts`). | Input to `npm run qa:sync-report`; do not edit by hand. |
| **`reports/unit-results.json`** | **Generated** (gitignored) | Vitest JSON when run with `--reporter=json --outputFile …`. | Same as above. |
...
| **`projects/`** | Mixed | `README.md`, `_template/` (template Playwright project), `0colors/` (older specs, `MANUAL-TEST-CASES.md`, `meta.ts`). | Legacy / optional runs; **primary** app E2E is `packages/frontend/tests/e2e/*.spec.ts`. |
...
| Location | Role |
|----------|------|
| **`playwright.config.ts`** (repo root) | E2E `testDir` = `packages/frontend/tests/e2e`, JSON → `QA-automation/reports/e2e-results.json`, optional `webServer` (`npm run dev`). |
| **`packages/frontend/tests/e2e/*.spec.ts`** | **Primary** Playwright coverage. |
| **`packages/frontend/src/**/*.unit.test.ts`** | Vitest unit tests (see `packages/frontend/vitest.config.ts`). |
...
| **`scripts/qa-local-runner.mjs`** | Local HTTP on **`127.0.0.1:47841`**: unit → Playwright (`PLAYWRIGHT_SKIP_WEB_SERVER` + `BASE_URL`) → sync. |
...
| **`.gitignore`** | Ignores generated `latest-run.json`, `runs-history.json`, `e2e-results.json`, `unit-results.json`. |

---

## 3. Data flow (short)

1. **Unit** → `QA-automation/reports/unit-results.json` (Vitest JSON reporter).  
2. **E2E** → `QA-automation/reports/e2e-results.json` (Playwright JSON reporter).  
3. **`npm run qa:sync-report`** → reads both + `automation-overlays.json` → writes dashboard JSON, history, copies `TEST-CATALOG.md` into `public/qa-docs/`.  
4. **QA hub** fetches those static files and displays runs, tables, coverage notes, and text report.
...
- **Run all tests** — Vite dev only; calls `/__qa-runner/run` (requires `npm run qa:runner` or `npm run dev:qa`). Shows live phase, elapsed time, runner log, and clear errors if the runner is unreachable.
...
| E2E then sync in one shot | `npm run test:e2e:report` |
| Dev + backend + frontend + QA runner | `npm run dev:qa` |
| Runner only (second terminal) | `npm run qa:runner` |
| Trigger from browser | With dev app + runner up → QA hub **Run all tests** |
